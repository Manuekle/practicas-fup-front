...
const reglas = computed(() => ({
  ...
  agreement_type: {
    required: helpers.withMessage(
      "El tipo de convenio es obligatorio",
      required
    ),
  },
  scope: {
    required: helpers.withMessage(
      "El alcance del convenio es obligatorio",
      required
    ),
  },
}));
...
const crearConvenio = async () => {
  if(!hasPerm(PERMISSIONS.AGREGAR_CONVENIO)){
  ...
  loading.value = true;
  try {
    const response = await apiClient.post(CREAR_CONVENIO_URL, {
      ...form.value,
    });
    toastSuccess("Convenio creado correctamente");
    router.push({ name: "convenios_detalle", params: { id: response.id } });
  } catch (error) {
...
watch(documentos, (newValue) => {
  form.value.documents = newValue.map((element) => element.id);
});
</script>

<template>
  <Layout>
    <PageHeader title="Crear convenio" pageTitle="Convenios"></PageHeader>
    <BCard>
      <BForm class="p-2" @submit.prevent="crearConvenio">
        <BRow>
          <BCol sm="6" md="6">
            <BFormGroup
              class="mb-3"
          ...
          <BCol sm="12" md="12">
            ...
            >
              ...
              <b-form-checkbox-group
                ...
                name="documents"
              />
              <div v-if="v$.documents.$error" class="invalid-feedback d-block">
