...
import { LISTA_PROGRAMAS_URL, OBTENER_ESTUDIANTE_URL, ACTUALIZAR_ESTUDIANTE_URL } from "@/constants/urls";
...
const updateStudent = async () => {
  loading.value = true;
  try {
    const url = ACTUALIZAR_ESTUDIANTE_URL(studentId)
...
const getStudent = async ()=>{
    loading.value = true
    try {
        const url = OBTENER_ESTUDIANTE_URL(studentId)
...
<template>
  <Layout>
    <PageHeader title="Actualizar Estudiante" pageTitle="Estudiantes" />
    <BCard>
      <BForm class="p-2" @submit.prevent="updateStudent">
        <BRow>
          <BCol sm="12" md="6">
            <BFormGroup
              class="mb-3"
