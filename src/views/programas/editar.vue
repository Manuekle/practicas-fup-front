...
import {
  ACTUALIZAR_PROGRAMA_URL,
  OBTENER_PROGRAMA_URL,
} from "@/constants/urls";
...
const updateProgram = async () => {
  loading.value = true;
  try {
    const url = ACTUALIZAR_PROGRAMA_URL(programId);
...
const getProgram = async () => {
  ...
  try {
    const url = OBTENER_PROGRAMA_URL(programId);
...
<template>
  <Layout>
    <PageHeader title="Actualizar Programa" pageTitle="Programas" />
    <BCard>
      <Loader :loading="loadingProgram" :transparent="false">
        <BForm class="p-2" @submit.prevent="updateProgram">
          <BRow>
            <BCol sm="12" md="6">
              <BFormGroup
                class="mb-3"
