<template>
  <Layout>
    <PageHeader title="Empresas" pageTitle="Listar empresas" />
    <BCard>
      <BCardText>
...
import { LISTA_EMPRESAS_URL } from "@/constants/urls";

const route = useRoute();
const router = useRouter();

const fields = [
  { key: "nui", label: "Número único de identificación" },
  { key: "name", label: "Nombre" },
  { key: "phone", label: "N. Teléfono" },
...
const getCompanies = async () => {
  ...
  try {
    actualizarQuery();
    const response = await apiClient.get(LISTA_EMPRESAS_URL, getParams());
...
watch(currentPage, getCompanies);
</script>
