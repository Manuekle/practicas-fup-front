<template>
  <Layout>
    <PageHeader title="Sustentaciones" pageTitle="Listar Sustentaciones" />
    <BCard>
      <BCardText>
        <div class="row g-2 align-items-center mb-3">
          ...
          <div class="col-12 col-md-4">
            <div class="input-group">
              ...
              <input
                ...
                @keyup.enter="buscar"
                class="form-control"
                placeholder="Buscar sustentación"
              />
            </div>
          </div>
          ...
          <div class="col-12 col-md d-flex justify-content-md-end gap-2">
            ...
            <router-link
              :to="{ name: 'registrar_sustentacion' }"
              class="btn btn-primary"
            >
              <i class="mdi mdi-plus"></i> Nueva Sustentación
            </router-link>
          </div>
        </div>
        ...
        <Loader :loading="loading" :transparent="false">
          ...
          >
            ...
            <template #cell(students)="data">
              <span v-for="(student, index) of data.item.students" :key="index">
                {{ student }} <br />
              </span>
            </template>
            <template #cell(result)="data">
              <span>
                {{ data.item.result ? data.item.result : "No asignada" }}
              </span>
            </template>
                  ...
                  <BPopover :delay="{ show: 0, hide: 0 }">
                    <template #target>
                      <router-link
                        ...
                        class="btn btn-sm btn-soft-primary"
                        v-if="data.item.status != 'COMPLETADA'"
                      >
...
import { LISTA_SUSTENTACIONES_URL } from "@/constants/urls";
...
const fields = [
  ...
  { key: "academic_session", label: "Jornada" },
...
const getdefenses = async () => {
  ...
  try {
    actualizarQuery();
    const response = await apiClient.get(LISTA_SUSTENTACIONES_URL, getParams());
    defenses.value = response.results;
