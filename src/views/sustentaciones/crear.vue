...
const getErrorMessage = (error) => {
  ...
  const posiblesCampos = ["scheduled_date", "application_date", "teachers", "students", "period", "place", "academic_session"];
...
const createDefense = async () => {
  ...
  try {
    ...
    const response = await apiClient.post(CREAR_SUSTENTACION_URL, data);
    toastSuccess("Sustentación registrada correctamente");
    router.push({ name: "detalle_sustentacion", params: { id: response.id } });
  } catch (error) {
...
<template>
  <Layout>
    <PageHeader title="Crear Sustentación" pageTitle="Sustentaciones" />
    <BCard>
      <BForm class="p-2" @submit.prevent="createDefense">
        <BRow>
          <BCol md="6">
            <BFormGroup
          ...
          <BCol md="12">
            <BRow>
              <BCol class="mb-3" md="6" v-for="teacher in form.teachers" :key="teacher.id">
                <div class="mb-2">
