<template>
  <Layout>
    <PageHeader title="Docentes" pageTitle="Listar Docentes" />
    <BCard>
      <BCardText>
        <div class="row g-2 align-items-center mb-3">
          ...
          <div class="col-12 col-md d-flex justify-content-md-end gap-2">
            ...
            <router-link
              :to="{ name: 'registrar_docentes' }"
              class="btn btn-primary"
            >
              <i class="mdi mdi-plus"></i> Nuevo Docente
            </router-link>
          </div>
        </div>
...
import { LISTA_DOCENTE_URL } from "@/constants/urls";
...
const getTeachers = async () => {
  ...
  try {
    actualizarQuery();
    const response = await apiClient.get(LISTA_DOCENTE_URL, getParams());
